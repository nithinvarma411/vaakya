...
class QwenToolCallParser(BaseToolCallParser):
    ...
    def __init__(self, *args, tool_call_start_token: str = "<tool_call>", tool_call_end_token: str = "</tool_call>", **kwargs):
        super().__init__(
            *args, 
    ...
    def parse_tool_calls(self, content: str) -> Tuple[str, List[ToolCall]]:
        ...
        tool_call_pattern = rf"{re.escape(start_token)}\s*(.+?)\s*{re.escape(end_token)}"
        ...
        for match in re.finditer(tool_call_pattern, content, re.IGNORECASE | re.DOTALL):
            ...
            try:
                ...
                if function_name:
                    ...
                    log.debug(f"Parsed tool call: {function_name} with args: {function_args}")
                
            except json.JSONDecodeError as e:
                log.warning(f"Failed to parse tool call JSON: {tool_json_str}, error: {e}")
                continue
        
        # Remove tool call XML from content if we found any
        if tool_calls:
            cleaned_content = re.sub(tool_call_pattern, "", content, flags=re.IGNORECASE | re.DOTALL).strip()
