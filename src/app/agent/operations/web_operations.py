...
class WebOperations:
    ...
    def _get_ddgs(self):
        ...
        if DDGS is None:
            raise ImportError("ddgs package not found. Please install with: pip install ddgs")
        
        if self._ddgs is None:
            self._ddgs = DDGS()
        return self._ddgs
    
    def search_web(self, query: str, max_results: int = 5) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                print(f"   {formatted_result['snippet'][:200]}{'...' if len(formatted_result['snippet']) > 200 else ''}")
    ...
    def search_news(self, query: str, max_results: int = 5) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                print(f"\n{i}. {formatted_result['title']}")
                print(f"   Source: {formatted_result['source']} | Date: {formatted_result['date']}")
                print(f"   {formatted_result['url']}")
                print(f"   {formatted_result['snippet'][:200]}{'...' if len(formatted_result['snippet']) > 200 else ''}")
    ...
    def search_images(self, query: str, max_results: int = 5) -> Optional[List[Dict[str, Any]]]:
        """Search for images."""
        try:
            ddgs = self._get_ddgs()
            ...
            for i, result in enumerate(results, 1):
                ...
                print(f"\n{i}. {formatted_result['title']}")
                print(f"   Size: {formatted_result['width']}x{formatted_result['height']}")
    ...
    def search_videos(self, query: str, max_results: int = 5) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                print(f"\n{i}. {formatted_result['title']}")
                print(f"   Publisher: {formatted_result['publisher']} | Duration: {formatted_result['duration']}")
                print(f"   {formatted_result['url']}")
                print(f"   {formatted_result['description'][:200]}{'...' if len(formatted_result['description']) > 200 else ''}")
    ...
    def quick_answer(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            if search_results:
                ...
                if snippet:
                    print(f"✅ Quick answer: {snippet[:300]}{'...' if len(snippet) > 300 else ''}")
                    return snippet
