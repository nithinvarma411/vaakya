    ...
    for i, query in enumerate(test_queries, 1):
        ...
        try:
            ...
            for msg in messages:
                ...
                if (str(msg.role) == "function" or 
                    str(msg.role) == "ChatRole.FUNCTION" or
                    (hasattr(msg, 'tool_calls') and msg.tool_calls) or
                    (msg.content and ("✅" in str(msg.content) or "🔍" in str(msg.content) or "📁" in str(msg.content)))):
                    function_evidence = True
                    break
            
            if function_evidence:
                print(f"✅ SUCCESS! Functions executed (evidence found)")
                successful_tests += 1
            else:
